...
from model_onlyNecessary import Transformer
...
def generate(prompts: List[str], model: Transformer, tokenizer: Tokenizer, *, max_tokens: int, chunk_size: int = None,
    ...
    cache = RotatingBufferCache(model.args.n_layers, model.args.max_batch_size, cache_window, model.args.n_kv_heads,
                                model.args.head_dim)
    cache.to(device="cuda", dtype=model.dtype)
    cache.reset()
...
if __name__ == "__main__":
    ...
    loaded = torch.load(model_path + 'consolidated.00.pth', mmap=True)
    with open(folder / 'params.json', 'r') as f:
        model_args = ModelArgs(**json.loads(f.read()))
    model_args.max_batch_size = max_batch_size

    transformer = Transformer(model_args, weights_mmap=loaded)
